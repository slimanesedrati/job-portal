...
const JobCard: React.FC<SafeJobType> = props => {
    // const [jobDetailIsOpen, setjobDetailIsOpen] = useState(false)

    const { id, title, salary, created_at, description, company, offer_type, sector } = props

    return (
        <div className="relative border border-gray-300 flex flex-col space-y-3 p-5 duration-200 hover:border-primary rounded-xl hover:shadow-md bg-white">
            <Link
                href={`/jobs/${id}`}
            ...
            >
                ...
                <Avatar withBaseUrl name={company?.name} logo_url={company?.logo} />
